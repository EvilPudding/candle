...
void c_renderer_update_lights(c_renderer_t *self)
{
...
void c_renderer_update_materials(c_renderer_t *self)
{
...
{
	if(!entity_exists(self->camera)) return;
	c_camera_t *camera = c_camera(&self->camera);

	struct gl_camera *glcamera = &self->scene.camera;

	c_camera_update_view(camera);

	glcamera->model = camera->model_matrix;
	glcamera->inv_model = camera->view_matrix;
	glcamera->pos = camera->pos;

	mat4_t inv_projection = mat4_invert(camera->projection_matrix);
	glcamera->projection = camera->projection_matrix;
	glcamera->inv_projection = inv_projection;
	glcamera->exposure = camera->exposure;
	/* printf("cam_pos "); vec3_print(camera->pos); */
}

void c_renderer_update_passes(c_renderer_t *self)
{
	int i;
	for(i = 0; i < self->passes_size; i++)
	{
		pass_t *pass = &self->passes[i];
		self->scene.passes[i].screen_size = vec2(
				pass->output->width,
				pass->output->height);
	}
}

int c_renderer_update(c_renderer_t *self)
{
	c_renderer_update_camera(self);
	c_renderer_update_materials(self);
	c_renderer_update_lights(self);
	c_renderer_update_passes(self);
...
void c_renderer_set_output(c_renderer_t *self, unsigned int hash)
{
	...
	/* TODO */
	/* texture_bind(self->output, -1); glerr(); */
	/* glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); glerr(); */
	/* glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); glerr(); */
}
...
static int c_renderer_gl(c_renderer_t *self)
{
	...
	c_renderer_add_pass(self, "gbuffer", "gbuffer", sig("visible"),
			PASS_CLEAR_DEPTH | PASS_CLEAR_COLOR,
			c_renderer_tex(self, ref("gbuffer")),
			c_renderer_tex(self, ref("gbuffer")),
		(bind_t[]){ {BIND_NONE} }
	);

	c_renderer_add_pass(self, "selectable", "select", sig("selectable"),
			PASS_CLEAR_DEPTH | PASS_CLEAR_COLOR,
			c_renderer_tex(self, ref("selectable")),
			c_renderer_tex(self, ref("selectable")),
		(bind_t[]){ {BIND_NONE} }
	);

	/* DECAL PASS */
	c_renderer_add_pass(self, "decals_pass", "decals", sig("decals"),
			PASS_LOCK_DEPTH,
	...
	c_renderer_add_pass(self, "ssao_pass", "ssao", sig("quad"), 0,
	...
	c_renderer_add_pass(self, "render_pass", "phong", sig("light"),
			PASS_ADDITIVE | PASS_CLEAR_COLOR,
			c_renderer_tex(self, ref("rendered")), NULL,
		(bind_t[]){
			{BIND_TEX, "gbuffer", .buffer = c_renderer_tex(self, ref("gbuffer"))},
			{BIND_NONE}
		}
	...
	c_renderer_add_pass(self, "refraction", "copy", sig("quad"), 0,
			c_renderer_tex(self, ref("refr")), NULL,
		(bind_t[]){
	...
	c_renderer_add_pass(self, "transp", "transparency", sig("transparent"), 0,
			c_renderer_tex(self, ref("rendered")),
			c_renderer_tex(self, ref("gbuffer")),
		(bind_t[]){
	...
	c_renderer_add_pass(self, "final", "ssr", sig("quad"), PASS_CLEAR_COLOR,
	...
	/* c_renderer_set_output(self, ref("gbuffer")); */
...
REG()
{
	...
	ct_listener(ct, WORLD, ref("window_resize"), c_renderer_resize);

	ct_listener(ct, WORLD | 100, ref("world_draw"), c_renderer_draw);
	ct_listener(ct, WORLD, ref("world_update"), c_renderer_update);

	ct_listener(ct, ENTITY, ref("entity_created"), c_renderer_created);

	ct_listener(ct, WORLD, ref("component_menu"), c_renderer_component_menu);

	ct_listener(ct, WORLD | 100, ref("mouse_press"), c_renderer_mouse_press);
	ct_listener(ct, WORLD | 100, ref("mouse_release"), c_renderer_mouse_release);
